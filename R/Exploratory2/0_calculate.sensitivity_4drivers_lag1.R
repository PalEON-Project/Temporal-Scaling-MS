...
paleon.gams.models <- function(data, response, k, predictors.all, site.effects){
	...
	if(substr(model.name,1,3)=="lin") {
		...
		gam1 <- gam(Y ~ s(tair, k=k) + s(precipf, k=k) + Y.lag + Site -1, data=data, correlation=corARMA(form=~Year|Site, p=1))
	} else {
		predictors <- c("tair", "precipf", "swdown", "CO2")
		gam1 <- gam(Y ~ s(tair, k=k) + s(precipf, k=k) + s(swdown, k=k) + s(CO2, k=k) + Y.lag + Site -1, data=data, correlation=corARMA(form=~Year|Site, p=1))
	}
	...
	if(substr(model.name,1,3)=="lin") {
		predictors <- c("tair", "precipf")
		gam1 <- gam(Y ~ s(tair, k=k) + s(precipf, k=k) + Y.lag, data=data, correlation=corARMA(form=~Year, p=1))
	} else {
		predictors <- c("tair", "precipf", "swdown", "CO2")
		gam1 <- gam(Y ~ s(tair, k=k) + s(precipf, k=k) + s(swdown, k=k) + s(CO2, k=k) + Y.lag, data=data, correlation=corARMA(form=~Year, p=1))
	}
	...
	# ----------------------------------------
	mod.out <- process.gamm(gamm.model=gam1, data=data, model.name=model.name, extent=extent, resolution=resolution, response=response, vars=c(predictors, "Y.lag"), write.out=F, outdir=out.dir, fweights=T, ci.model=T, ci.terms=T)
	# ----------------------------------------
