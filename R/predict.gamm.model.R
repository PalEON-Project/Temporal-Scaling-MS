model.gam <- function(data, model, response, scale="", k=4, outdir, 
	...
	# -----------
	if(fweights==T){	
		f.weights <- factor.weights(model=gam1, newdata=data.temp, sites=T); 
	...
	# -----------
	if(ci.model==T){
		ci.response <- post.distns(model=gam1, newdata=data.temp, terms=F, sites=T)
	...
	if(ci.terms==T){
		...
		ci.terms.pred <- post.distns(model=gam1, newdata=new.dat, terms=T, sites=T)

		out[["ci.terms"]] <- ci.terms.pred 
	}	
	# -----------
		
	save(out, file=file.path(outdir, paste("gamm", model, response, scale, "AllSites", "Rdata", sep=".")))
	return(out)
	# -----------
