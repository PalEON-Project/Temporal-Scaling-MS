...
paleon.gams.models <- function(data, response, k, predictors.all, comp.effects){
	...
	if(substr(model.name,1,3)=="lin") {
		...
		gam1 <- gam(Y ~ s(tair, by=Site, k=k) + s(precipf, by=Site, k=k), data=data, correlation=corARMA(form=~Year|Site, p=1))
	} else {
		predictors <- c("tair", "precipf", "CO2")
		gam1 <- gam(Y ~ s(tair, by=Site, k=k) + s(precipf, by=Site, k=k) + s(CO2, by=Site, k=k), data=data, correlation=corARMA(form=~Year|Site, p=1))
	}
