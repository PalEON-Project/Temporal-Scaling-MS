...
paleon.gams.models <- function(data, response, k, predictors.all, site.effects){
	...
	if(substr(model.name,1,3)=="lin") {
		...
		gam1 <- gam(Y ~ s(tair, k=k) + s(precipf, k=k) + (Evergreen + Grass) * (Site -1), data=data, correlation=corARMA(form=~Year|Site, p=1))
	} else {
		predictors <- c("tair", "precipf", "CO2", "Evergreen", "Grass")
		gam1 <- gam(Y ~ s(tair, k=k) + s(precipf, k=k) + s(CO2, k=k) + (Evergreen + Grass) * (Site -1), data=data, correlation=corARMA(form=~Year|Site, p=1))
	}
	...
	if(substr(model.name,1,3)=="lin") {
		predictors <- c("tair", "precipf", "Evergreen", "Grass")
		gam1 <- gam(Y ~ s(tair, k=k) + s(precipf, k=k) + Evergreen + Grass, data=data, correlation=corARMA(form=~Year, p=1))
	} else {
		predictors <- c("tair", "precipf", "CO2", "Evergreen", "Grass")
		gam1 <- gam(Y ~ s(tair, k=k) + s(precipf, k=k) + s(CO2, k=k) + Evergreen + Grass, data=data, correlation=corARMA(form=~Year, p=1))
	}
	...
	# ----------------------------------------
	mod.out <- process.gamm(gamm.model=gam1, data=data, model.name=model.name, extent=extent, resolution=resolution, response=response, vars=predictors, write.out=F, outdir=out.dir, fweights=T, ci.model=T, ci.terms=T, PFT=F)
	# ----------------------------------------
