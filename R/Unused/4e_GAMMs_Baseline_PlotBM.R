# ----------------------------------------
# Objective: Create "_PlotBM" sensitivity curves for each model/data stream 
...
source('R/0_calculate.sensitivity_TPC_PlotBM.R', chdir = TRUE)
...
fig.dir <- file.path(fig.base, "Sensitivity_PlotBM")
dat.dir <- file.path(dat.base, "Sensitivity_PlotBM")
...
for(i in 1:length(models.base)){
	if(i==1) {
		...
		mod.out[[paste("gamm", names(models.base)[i], "_PlotBM", sep=".")]] <- models.base[[i]]$gamm
	} else {
...
save(mod.out, file=file.path(dat.dir, "gamm_PlotBM.Rdata"))
}
# -------------------------------------------------------------------------------
...
pdf(file.path(fig.dir, "GAMM_ModelFit_PlotBM.pdf"))
{
print(	
ggplot(data=mod.out$ci.response[!substr(mod.out$ci.response$Model, 1, 8)=="TreeRing",]) + facet_grid(PlotID~ Model, scales="free") + theme_bw() +
	...
	labs(title=paste("_PlotBM"), x="Year", y="NPP")
)
print(	
...
ggplot(data=mod.out$ci.response[mod.out$ci.response$Model=="TreeRingRW" & mod.out$ci.response$PlotID=="ME029",]) + facet_wrap(~ TreeID, scales="fixed") + theme_bw() +
	...
	scale_fill_manual(values=paste(col.model)) +
	scale_color_manual(values=paste(col.model)) +		
	labs(title=paste("_PlotBM"), x="Year", y="RW")
)
print(	
...
pdf(file.path(fig.dir, "GAMM_DriverSensitivity_PlotBM.pdf"), height=8.5, width=11)
{
print(
...
ggplot(data=mod.out$ci.terms[mod.out$ci.terms$Effect %in% c("Biomass"),]) + facet_wrap( ~ Model, scales="free_x") + theme_bw() +		
	geom_ribbon(aes(x=x, ymin=lwr, ymax=upr, fill=PlotID), alpha=0.5) +
	geom_line(aes(x=x, y=mean, color=PlotID), size=2) +
	geom_hline(yintercept=0, linetype="dashed") +
	scale_y_continuous(limits=c(-5, 5)) +
	scale_fill_manual(values=paste(col.model)) +
	scale_color_manual(values=paste(col.model)) +		
