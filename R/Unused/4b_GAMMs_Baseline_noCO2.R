# ----------------------------------------
# Objective: Create "NoCO2" sensitivity curves for each model/data stream 
...
source('R/0_calculate.sensitivity_TP.R', chdir = TRUE)
...
fig.dir <- file.path(fig.base, "Sensitivity_NoCO2")
dat.dir <- file.path(dat.base, "Sensitivity_NoCO2")
...
for(i in 1:length(models.base)){
	if(i==1) {
		...
		mod.out[[paste("gamm", names(models.base)[i], "NoCO2", sep=".")]] <- models.base[[i]]$gamm
	} else {
...
save(mod.out, file=file.path(dat.dir, "gamm_NoCO2.Rdata"))
}
# -------------------------------------------------------------------------------
...
pdf(file.path(fig.dir, "GAMM_ModelFit_NoCO2.pdf"))
{
print(	
ggplot(data=mod.out$ci.response[!substr(mod.out$ci.response$Model, 1, 8)=="TreeRing",]) + facet_grid(PlotID~ Model, scales="free") + theme_bw() +
	...
	labs(title=paste("NoCO2"), x="Year", y="NPP")
)
print(	
...
ggplot(data=mod.out$ci.response[mod.out$ci.response$Model=="TreeRingRW" & mod.out$ci.response$PlotID=="ME029",]) + facet_wrap(~ TreeID, scales="fixed") + theme_bw() +
	...
	scale_fill_manual(values=paste(col.model)) +
	scale_color_manual(values=paste(col.model)) +		
	labs(title=paste("NoCO2"), x="Year", y="RW")
)
print(	
...
pdf(file.path(fig.dir, "GAMM_DriverSensitivity_NoCO2.pdf"), height=8.5, width=11)
{
print(
ggplot(data=mod.out$ci.terms[mod.out$ci.terms$Effect %in% c("tair", "precipf"),]) + facet_wrap(~ Effect, scales="free_x") + theme_bw() +		
