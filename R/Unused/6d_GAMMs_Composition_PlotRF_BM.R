...
fig.dir <- file.path(fig.base, "Sensitivity_PFT_PlotRF_BM")
dat.dir <- file.path(dat.base, "Sensitivity_PFT_PlotRF_BM")
...
paleon.models[["TreeRingNPP"]]             <- plot.npp[,c("Site", "PlotID", "Year", "PFT")]
...
save(mod.out, file=file.path(dat.dir, "gamm_PFT_PlotRF_BM.Rdata"))
}
# -------------------------------------------------------------------------------
...
pdf(file.path(fig.dir, "GAMM_ModelFit_PFT_PlotRF_BM.pdf"))
{
print(	
...
pdf(file.path(fig.dir, "GAMM_DriverSensitivity_PFT_PlotRF_BM.pdf"), height=11, width=8.5)
{
m.order <- unique(mod.out$data[,"Model.Order"])
col.model <- c(paste(model.colors[model.colors$Model.Order %in% m.order,"color"]), "black", "gray30")
print(
