...
for(i in 1:length(sites)){
	...
	names(RA[[i]]) <- c(ncvar_get(auto.resp, "ModelNames"), "SiB")
	names(RH[[i]]) <- c(ncvar_get(hetero.resp, "ModelNames"), "SiB")
	names(SOILCARB[[i]]) <- c(ncvar_get(soilcarb, "ModelNames"), "SiB")
	names(SOILMOIST[[i]]) <- c(ncvar_get(soilmoist, "ModelNames"), "SiB")
	names(EVAP[[i]]) <- c(ncvar_get(evap, "ModelNames"), "SiB")
	names(TRANSP[[i]]) <- c(ncvar_get(transp, "ModelNames"), "SiB")
	
	row.names(GPP[[i]]) <- years
	row.names(AGB[[i]]) <- years
	row.names(TEMP[[i]]) <- years
	row.names(PRECIP[[i]]) <- years
	row.names(RA[[i]]) <- years
	row.names(RH[[i]]) <- years
	row.names(SOILCARB[[i]]) <- years
	row.names(SOILMOIST[[i]]) <- years
	row.names(EVAP[[i]]) <- years
	row.names(TRANSP[[i]]) <- years
...
for(i in 1:length(sites)){
	...
	df1 <- rbind(df1, cbind(gpp.df, agb.df$AGB, lai.df$LAI, npp.df$NPP, nee.df$NEE, ra.df$AutoResp, rh.df$HeteroResp, soilcarb.df$SoilCarb, soilmoist.df$SoilMoist, evap.df$Evap, transp.df$Transp))
	df1 <- merge(df1, climate.df, all.x=T, all.y=T)
	}
