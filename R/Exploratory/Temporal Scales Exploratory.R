...
for(s in unique(ecosys$Site)){
	...
	for(m in unique(ecosys$Model)){
		...
		ecosys[ecosys$Site==s & ecosys$Model==m, "GPP.dev"] <- (ecosys[ecosys$Site==s & ecosys$Model==m, "GPP"] - x.gpp)/x.npp
		...
		ecosys[ecosys$Site==s & ecosys$Model==m, "NEE.dev"] <- (ecosys[ecosys$Site==s & ecosys$Model==m, "NEE"] - x.nee)/x.npp
...
# summary(met.year)
summary(ecosys)
write.csv(ecosys, "phase1a_output_variables/PalEON_MIP_Yearly.csv", row.names=F)
...
summary(ecosys)
#summary(met.year)
write.csv(ecosys, "phase1a_output_variables/PalEON_MIP_Yearly_2_Roll.csv", row.names=F)
