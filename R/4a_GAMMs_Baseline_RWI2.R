...
fig.dir <- file.path(fig.base, "Sensitivity_Baseline_RWI")
dat.dir <- file.path(dat.base, "Sensitivity_Baseline_RWI")
...
# Add the data to paleon.models
paleon.models[["TreeRingRWI"]]             <- tree.rings[,c("Site", "PlotID", "TreeID", "Year")]
paleon.models$TreeRingRWI$Model            <- as.factor("TreeRingRWI")
paleon.models$TreeRingRWI$Model.Order      <- as.factor("Tree Ring RWI")
paleon.models$TreeRingRWI[,predictors.all] <- tree.rings[,paste0(predictors.all, predictor.suffix)]
paleon.models$TreeRingRWI$Y                <- tree.rings[,response]
# paleon.models$TreeRingRWI$Biomass          <- tree.rings[,biomass.mod]
paleon.models$TreeRingRWI$Biomass          <- 0
paleon.models$TreeRingRWI$Time             <- tree.rings[,time.mod]
paleon.models$TreeRingRWI$Resolution       <- tree.rings[,"Resolution"]

# Make sure everything is complete cases
paleon.models$TreeRingRWI <- paleon.models$TreeRingRWI[complete.cases(paleon.models$TreeRingRWI[,c(predictors.all, "Y", "Biomass", "Time")]),]

# Order everything the same way to make life easier
paleon.models$TreeRingRWI <- paleon.models$TreeRingRWI[,names(paleon.models[[1]])]
summary(paleon.models$TreeRingRWI)
