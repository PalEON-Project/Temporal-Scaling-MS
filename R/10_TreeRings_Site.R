...
detrend.only <- 	gam(Y ~ s(Year, by=TreeID, k=3, bs="cr"), data= tree.rings)


mod.tr <- paleon.gams.models(data=tree.rings2, response="RW", k=k, predictors.all=predictors.all, comp.effects=F)
save(mod.tr, file=file.path(dat.dir, "gamm_TreeRings_NPP_Site.Rdata"))
# -------------------------------------------------
...
pdf(file.path(fig.dir, "GAMM_TreeRingFit_NPP_Site.pdf"))
print(
...
pdf(file.path(fig.dir, "GAMM_TreeRing_DriverSensitivity_NPP_Site.pdf"))
...
ggplot(data=mod.tr$ci.terms[mod.tr$ci.terms$Effect=="tair",]) + facet_wrap( ~ Effect, scales="fixed") + theme_bw() +	
