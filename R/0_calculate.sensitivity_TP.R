...
paleon.gams.models <- function(data, k, predictors.all, PFT=F){
	...
	# ----------------------------------------
	if(PFT==T){
		predictors=c("tair", "precipf", "PFT", "Biomass",)
		...
		gam1 <- gam(Y ~  s(Biomass, bs="cr", k=3, by=PFT) + s(tair, k=k, by=PFT) + s(precipf, k=k, by=PFT), data=data)
	# ----------------------------------------
	} else {
	# ----------------------------------------
		predictors=c("tair", "precipf","Biomass")
		# gam1 <- gam(Y ~ s(Time, bs="cr", k=4) + s(Biomass, bs="cr", k=3) + s(tair, k=k) + s(precipf, k=k) + s(CO2, k=k), data=data)
		gam1 <- gam(Y ~ s(Biomass, bs="cr", k=3) + s(tair, k=k) + s(precipf, k=k), data=data)
	}
	# ----------------------------------------
