...
fig.dir <- file.path(fig.base, "Sensitivity_TempExtent_PlotRF_BM")
dat.dir <- file.path(dat.base, "Sensitivity_TempExtent_PlotRF_BM")
...
for(i in 1:length(models.base)){
	if(!length(models.base[[i]])>1) next
  if(i==1) {
		mod.out <- list()
...
pdf(file.path(fig.dir, "GAMM_ModelFit_TempExtent_PlotRF_BM.pdf"))
{
...
pdf(file.path(fig.dir, "GAMM_DriverSensitivity_TempExtent_PlotRF_BM.pdf"))
{
m.order <- unique(mod.out$data[,"Model.Order"])
col.model <- c(paste(model.colors[model.colors$Model.Order %in% m.order,"color"]), "black", "gray30")
print(
